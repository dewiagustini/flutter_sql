...
class DatabaseHelper {
  ...
  initDb() async {
    ...
    String path = join(documentsDirectory.path, "db_main.db");
  ...
  void _onCreate(Database db, int version) async {
    ...
    await db.execute(
        "CREATE TABLE User(id INTEGER PRIMARY KEY, firstname TEXT, lastname TEXT, dob TEXT, umur TEXT, email TEXT)");
  }
  ...
  Future<List<User>> getUser() async {
    ...
    for (int i = 0; i < list.length; i++) {
      var user =
      new User(list[i]["firstname"], list[i]["lastname"], list[i]["dob"], list[i]["umur"], list[i]["email"]);
